...
3. 细长四边形条带：若长边是真实自由边，以失败单元为种子沿两条短边追踪完整连续条带；即使中间四边形未被 HyperMesh 标为失败，也会作为受控支撑单元加入同一移动链。Python 对整条链的节点同时预模拟，不再把局部锯齿移动误判为相邻单元恶化。共享节点对相邻短边方向取折中，直线链合并为一个移动块，曲线链按约 6–10° 方向差和 15% 位移差分块，再用节点 mark 批量向边界外侧移动，每个节点每轮只移动一次。
...
5. 对由独立 component 表示的焊缝 shell，Tcl 会额外导出检查范围外的一环壳单元作为只读拓扑上下文，因此只选择焊缝 component 时 Python 仍能看到两侧相邻 component。Python 比较焊缝条带与两条长边相邻 component 的壳法向：只有一侧达到近似垂直（夹角至少 60°）时，沿焊缝短边方向平移该侧整串衔接节点；两侧都近似垂直时，两条节点链沿短边方向向外对称扩展。未失败的连续焊缝四边形也作为支撑单元参与整链模拟和移动。该识别允许跨越由焊缝转角产生的特征边保护，但用户明确固定的节点仍不可移动。
