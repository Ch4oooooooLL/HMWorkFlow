...
| Rollback/final guard | safer | `saveModelSnapshot`, `restoreSnapshot`, `qualityWorsened`, final block in `startOptimizationCore` | Restores only the current region on command/protection/recheck failure; final guard rolls accepted regions back one at a time. |
...
HyperMesh first classifies failures using the selected criteria. Python selects a conservative repair only after HyperMesh has declared the element failed; for expansion sizing it additionally reads the enabled `min length` and `aspect ratio` failure limits, while HyperMesh remains the final authority.
...
| `expand_internal_quad` | fully internal narrow 4-node shell | Four edges have exactly two owners, the opt-in setting is enabled, and criteria requires a larger short dimension. Python computes an independent minimum target for both opposite short sides. | Off by default; explicit/blocked anchors; stale topology; Python inversion/degeneracy simulation failure. | Moves all four corner nodes symmetrically; no intentional ID change. | Planner `_quad_actions`; Python `prevalidate_operation`; Tcl `symmetricExpansionTargets` and batched `*translatemark`. |
| `expand_triangle_short_edge` | moderate failed 3-node shell | The triangle does not meet the ultra-skinny collapse rule and its short edge is below the minimum implied by enabled criteria limits. | Explicit/blocked anchors; stale edge; Python inversion/degeneracy simulation failure. | Moves the two short-edge endpoints symmetrically; no intentional ID change. | Planner `plan_optimization_actions`; Python `prevalidate_operation`; Tcl `symmetricExpansionTargets` and batched `*translatemark`. |
...
```text
collapse_short_edge -> expansion actions -> split_quad -> manual_review
```
...
- Completion always performs a final whole-scope native quality check. New failures outside optimized regions or growth beyond the topology-replacement allowance cause the latest accepted region to be restored and the guard to run again; earlier safe regions remain applied.
...
| `expand_internal_quad` / `expand_triangle_short_edge` | Moves nodes only. | Reads source nodes, target distances and every shell incident to a moving node. | Python rejects inversion/degeneracy before batching; read/write and one-ring conflicts keep overlapping movements in separate batches. |
