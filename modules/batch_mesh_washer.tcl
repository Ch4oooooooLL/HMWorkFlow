...
proc ::BatchMeshWasher::showRules {} {
    ...
    tk_messageBox -icon info -title [::HWFlow::txt "Sheet BatchMesh and Washer Rules" "Sheet BatchMesh and Washer Rules"] -message $msg
}
...
proc ::BatchMeshWasher::showPanel {} {
    ...
    wm title $w "[::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] v$VERSION"
    wm resizable $w 0 0

    frame $w.main -padx 12 -pady 10
    pack $w.main -fill both -expand 1

    label $w.main.title -text [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -font [::HWFlow::uiFont heading]
...
proc ::BatchMeshWasher::acceptPanel {} {
    variable ui
    if {[llength $ui(selectedComps)] == 0} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "请先选择组件。" "Pick components first."]
        return
    }
    foreach key {ELEM_SIZE MIN_ELEM_SIZE MAX_ELEM_SIZE FEATURE_ANGLE HOLE_MIN_DIA HOLE_MAX_DIA} {
        if {![string is double -strict $ui($key)]} {
            tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message "$key must be a number."
            return
        }
    }
    foreach key {SURFACE_BATCH_SIZE WASHER_PROGRESS_STEP WASHER_BATCH_SIZE} {
        if {![string is integer -strict $ui($key)]} {
            tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message "$key must be an integer."
            return
        }
    }
    ...
    if {$ui(ELEM_SIZE) <= 0 || $ui(MIN_ELEM_SIZE) <= 0 || $ui(MAX_ELEM_SIZE) <= 0 || $ui(MIN_ELEM_SIZE) > $ui(MAX_ELEM_SIZE)} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "网格尺寸参数无效。" "Invalid mesh size parameters."]
        return
    }
    if {$ui(HOLE_MIN_DIA) < 0 || $ui(HOLE_MAX_DIA) <= $ui(HOLE_MIN_DIA)} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "孔径参数无效。" "Invalid hole diameter parameters."]
        return
    }
    if {$ui(WASHER_PROGRESS_STEP) < 1} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "washer 进度步长必须大于 0。" "Washer progress step must be greater than 0."]
        return
    }
    if {$ui(SURFACE_BATCH_SIZE) < 1} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "surface 分块数量必须大于 0。" "Surface batch size must be greater than 0."]
        return
    }
    if {$ui(WASHER_BATCH_SIZE) < 1} {
        tk_messageBox -icon warning -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message [::HWFlow::txt "washer 批量孔数必须大于 0。" "Washer batch size must be greater than 0."]
        return
    }
    ::BatchMeshWasher::saveState
    set ui(ok) 1
...
proc ::BatchMeshWasher::main {} {
    ...
    if {[llength [info commands ::HWFlow::progressOpen]] > 0} {
        set progressOpened [::HWFlow::progressOpen \
            [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] \
    ...
    set code [catch {
        ::BatchMeshWasher::msg [::HWFlow::txt "==== Sheet BatchMesh and Washer started ====" "==== Sheet BatchMesh and Washer started ===="]
        if {$progressOpened} {
    ...
    ::BatchMeshWasher::saveState
    if {$progressOpened && [llength [info commands ::HWFlow::progressClose]] > 0} {
        catch {::HWFlow::progressClose [::HWFlow::txt "Sheet BatchMesh and Washer finished." "Sheet BatchMesh and Washer finished."] 100.0}
    }
    tk_messageBox -icon info -title [::HWFlow::txt "Sheet BatchMesh and Washer" "Sheet BatchMesh and Washer"] -message $msg
}
