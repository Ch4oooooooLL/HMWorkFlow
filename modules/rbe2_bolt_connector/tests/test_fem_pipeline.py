...
class TclImportContractTests(unittest.TestCase):
    def test_executor_uses_same_session_fe_import_and_has_diagnostic_stages(self):
        ...
        self.assertIn("::HWFlow::runHyperMeshIo import", source)
        self.assertIn("[list *feinputwithdata2 $reader", source)
        self.assertIn("[file nativename $incrementalFem] 1", source)
