...
def calculate(req,model):
    ...
    source=s["source_node_ids"]; matched=match(source,s["candidate_node_ids"],model.nodes); target,reversed_path,offset,pairing_cost=align_with_metadata(source,matched,model.nodes,s["closed_loop"]); validate(target,model); element_plan=plan(source,target,s["closed_loop"],model.nodes)
