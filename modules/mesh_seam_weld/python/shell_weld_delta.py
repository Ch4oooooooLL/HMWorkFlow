...
def _plan_lines(plan, include_name=True):
    ...
    if replacements:
        target_component=int(replacements[0]["component_id"]); lines.append("$HMCOMP ID {}".format(target_component))
        for element in replacements:
            if int(element["component_id"]) != target_component: raise FemDeltaError("replacement elements span multiple components")
