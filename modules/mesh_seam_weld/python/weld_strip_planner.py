...
def plan_zipper(source, target, nodes, closed=False, allow_end_tria=True, max_tria_ratio=0.15):
    ...
    if closed and (len(source) != len(target)):
        raise ValueError("closed unequal-node zipper requires manual review")
    ...
    while i < source_limit or j < target_limit:
        next_s = source_t[(i+1) % len(source)] if i < source_limit else float("inf")
        next_t = target_t[(j+1) % len(target)] if j < target_limit else float("inf")
