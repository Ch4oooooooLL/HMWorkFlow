...
与 `examples/AutoShellSeamBackend/test_fem/` 的验收夹具不同，本生成器可重复铺设隔离场景，
默认生成 100 份矩阵（70,100 节点 / 47,000 壳单元 / 2,200 组件），用于 CLI 性能和实机流程验证。
...
可用 `--copies N` 调整容量；默认 `100`，快速检查可用 `--copies 1`。
...
下列命令会生成大 FEM，然后依次调用生产 `main.py` 完成 detect 和 plan，检查候选缓存、
READY 计划、修改后 FEM、remesh plan 与 transfer manifest，并执行 120 秒性能门禁：

```powershell
runtime\python\windows-x64\python.exe examples\FemAutoSeam_Validation\run_cli.py
```

默认产物位于 `runtime/tasks/fem_auto_seam/large_cli_validation/`，最终结论见
`validation_report.json`。`--workers 0` 表示按 CPU 自动选择进程数，也是推荐值；
`--max-seconds 0` 可仅记录耗时而不执行性能门禁。

## 模型结构

10 组场景沿全局 X 轴排布，间距 300 mm。壳组件默认厚度 1.0~2.0 mm（`_T1`/`_T2` 命名），
主网格单元 10 mm。焊缝处源/目标组件不共享节点（装配语义），F10 为共享节点负向对照。
默认间隙 3 mm（< `search_distance` 12）。

| 场景 | 结构 | 预期 |
|---|---|---|
| F01 | 直 T：底板 90x60 + 筋板 60x20（gap 3） | 1 高置信 T 候选，自动 |
| F02 | 斜 T：筋板沿 +y 倾斜 30° | 1 T 候选，自动 |
...
- 场景间距 300 mm（夹具 500 mm 间隔）；默认重复 100 份，规模为 70,100 节点 / 47,000 壳单元；
